...
Koska Pyramidissa MVC-arkkitehtuurin määrittely on hyvin epävakaalla pohjalla, tulee tehdä selkeä oletus komponenttejen jaottelulle. Esimerkiksi \emph{views.py} ja \emph{template.pt} -tiedostot on tarkoitettu toimimaan yhdessä näkymänä. Tiedosto \emph{views.py}
sisältää kuitenkin niin paljon ohjaimelle yhteisiä piirteitä, joten se erottuu selvästi \emph{template.pt} -tiedostosta.  Tarkastelussa oletetaan, että  \emph{models.py} sisältää mallin, \emph{views.py} ohjaimen ja \emph{template.pt} näkymän. 
...
toteutetaan eri tavalla. Pyramidin MVC-komponenttejen kommunikointi voidaan esittää visuaalisesti kommunikaatiomallilla, jossa poistetaan osia Krasnerin alkuperäisestä mallista \cite{Krasner:desc}. 
...
\caption{Pyramidin kommunikointi MVC-komponenttejen kesken. Kuva on muokattu Krasnerin esittelemästä kommunikointimallista.  }
\end{figure}
...
todeta, että ongelmaksi ei muodostu yksittäiset komponentit, vaan 
